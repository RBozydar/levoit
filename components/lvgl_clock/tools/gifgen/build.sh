...
# component's own .esphome build tree.
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
COMPONENT="$(cd "$HERE/../.." && pwd)"          # components/lvgl_clock
SHIM="$HERE/shim"
CONFDIR="$HERE"                                  # holds lv_conf.h

# LVGL source, from any of the component's esphome build trees.
LV="$(ls -d "$COMPONENT"/.esphome/build/*/managed_components/lvgl__lvgl 2>/dev/null | head -1)"
if [[ -z "${LV:-}" || ! -f "$LV/lvgl.h" ]]; then
  echo "error: LVGL source not found under .esphome/build/*/managed_components/lvgl__lvgl" >&2
  echo "       run 'esphome config example_analog.yaml' once to populate it." >&2
  exit 1
fi
